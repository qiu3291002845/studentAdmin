<template>
    <div>
        <!-- 搜索框与新建 -->
        <el-row>
            <el-button style="float: left;" type="primary" @click="newUser">新建</el-button>
            <!-- 搜索功能 -->
            <el-input v-model="search" @input="inputSearch" class="Search" placeholder="请输入内容" suffix-icon="el-icon-search"></el-input>
        </el-row>
        <!-- 列表框 -->
        <el-table border :data="tableData" style="width: 100%;margin-top: 20px;" v-loading="loading" width>
            <!-- 头像 -->
            ...
            <el-table-column align="center" label="角色" width="130">
                <template slot-scope="scope">
                    <span>{{scope.row.role === null ?"未知":scope.row.role.type === 0 ?"学生":"老师"}}</span>
                </template>
            </el-table-column>
            <!-- 注册时间 -->
            <el-table-column class="el-icon-date" align="center" prop="time" label="注册时间" width="210">
            </el-table-column>
            <!-- 邮箱 -->
